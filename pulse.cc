    ...
    state_(PA_CONTEXT_UNCONNECTED)
{
  ...
  pa_proplist_sets(connect_props_, PA_PROP_APPLICATION_ID, "com.iskrembilen.patoggler");
  pa_proplist_sets(connect_props_, PA_PROP_APPLICATION_VERSION, "lol");
...
void PulseClient::setMute(const Sink *sink, const bool mute) {
  if (!sink) {
    puts("No sink to mute");
    return;
  }
  pa_operation* op = pa_context_set_sink_mute_by_index(context_, sink->Index(), int(mute), NULL, NULL);
  if (!wait_for_op(op)) {
    puts("Failed to wait for mute op");
  }
}
